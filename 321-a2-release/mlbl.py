...
class MLBL(object):
    ...
    def forward(self, params, X, Im, test=True):
        ...
        ########################################################################
        r_hat_left=np.array([np.zeros(R.shape[0])])
        r_hat_right =np.dot(np.maximum(np.dot(Im,J)+bj,0),M)
        
        for i in range(batchsize):
            c_times_rw = np.dot(C[0].T,R.T[X[i][0]])
            for j in range(1,X[0].shape[0]):
                current_crw = np.dot(C[j].T,R.T[X[i][j]])
                c_times_rw = np.vstack((c_times_rw,current_crw))
            r_hat_left = np.vstack((r_hat_left,sum(c_times_rw)))
        
        r_hat_left = r_hat_left[1:]     
        
        r_hat = r_hat_left+r_hat_right
        pred_numerator = np.exp(np.dot(r_hat,R)+bw)
        preds = pred_numerator/(np.sum(pred_numerator,axis=1).reshape(pred_numerator.shape[0],1))
    ...
    def backward(self, params, X, Im, Y):
        ...
        grad_params = grad(self.compute_obj)(params,X,Im,Y)
        ########################################################################
