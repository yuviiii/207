...
void new_connection(int listenfd) {
  ...
  socklen_t socklen = sizeof(new_client_addr);
  // in case that accept operation fails
  if ((new_client_fd = accept(listenfd, (struct sockaddr *) &new_client_addr,
      &socklen)) < 0) {
    perror("accept");
    exit(1);
  } else {
    // Adds this new client to clientlist.
    add_client(new_client_fd);
    // Initializes new client struct.
    // Since the username has not been typed, sets it to empty
    clientlist->name = NULL;
    clientlist->buf = (char *) malloc(
    INPUT_BUFFER_SIZE * sizeof(char));
    clientlist->inbuf = 0;
    clientlist->room = sizeof(char) * INPUT_BUFFER_SIZE;
    clientlist->after = clientlist->buf;
    clientlist->original_buf_pointer = clientlist->buf;
    write(new_client_fd, "What is your user name?\r\n", 20);
  }
}

void send_message_to_target_client(char* name, char * buf) {
  Client *current_client = clientlist;
  while (current_client != NULL) {
    // Such a client is found.
    if (strcmp(current_client->name, name) == 0) {
      write(current_client->fd, buf, strlen(buf));
    }
    // Indicator points to the next client.
...
void execute_command(int fd, char *command){
  ...
  char *post = "post";
    ...
    // when command is post <target_name> <message>.
    if (command[0]=='p'){
      char subbuff[5];
      memcpy( subbuff, &command[0], 4 );
      subbuff[4] = '\0';
      if (strcmp(subbuff,post)){
      if (clientlist->testdone){
        printf("mmpig\r\n");
